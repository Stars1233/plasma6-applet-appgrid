/*
    ...
    user-visible scenarios that v1.7.9 handled wrongly:

      A. Icon file replaced on disk (issue #86) — user edits an app
         icon via the menu editor or a package upgrade rewrites
         /usr/share/icons/.../<app>.png. The .desktop file may or may
         not be touched. Qt's pixmap cache must drop the stale bitmap
         so the new one appears without restart. This was the original
         reason QIcon::setThemeName(QIcon::themeName()) was added to
         AppModel::reload().

      B. System icon-theme switch (issue #103) — user picks a new
         theme in System Settings → Icons. The fix for #86 regressed
         this case: setThemeName(x) sets an *override* on Qt's icon
         engine, locking the theme name across the entire plasmashell
         process and blocking subsequent platform theme changes from
         propagating until logout.
...
private slots:
    ...
    /// that signal for either path:
    ///   * #86: KIconLoader::emitChange() called by kbuildsycoca6,
    ///     gtk-update-icon-cache, kupdateiconcache, or the menu editor
    ///     after writing the .desktop.
...
void AppModelTest::iconChangedEmitsDataChangedOnIconRole()
{
    ...
    // QML signals are also Qt signals — invoke iconChanged via meta-
    // object to fake what KIconLoader does when its cache flushes.
